package com.example.pertemuan10_71190496
    ...
    fun search(nama: String, usia: String){
        ...
        with(cursor){
            while (this?.moveToNext() == true) {
                val nama = this?.getColumnIndex(DatabaseContract.Penduduk.COLUMN_NAME_NAMA)?.let {
                    getString(
                        it
                    )
                }
                val usia = this?.getColumnIndex(DatabaseContract.Penduduk.COLUMN_NAME_USIA)?.let {
                    getString(
                        it
                    )
                }
